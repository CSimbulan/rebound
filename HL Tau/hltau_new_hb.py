...
def heartbeat(reb_sim):
    ...
    global seed_start, p_mass
    heartbeatPath = "HB Files/{1}/heartbeat_seed{0}_{1}.txt".format(seed_start, p_mass)
    sim = reb_sim.contents
    global ctr
    if sim.dt < 1.0e-10:
        if ctr == 1000:
            hbFile = open(heartbeatPath, "a")
            r = calculateDistance(sim, "distance")
            hbFile.write("{0}\t{1}\t{2}\n".format(sim.t, r, sim.dt_last_done))
            print "Sim.dt dropped: ", sim.t, r, sim.dt_last_done
            ctr = 0
        else:
            ctr += 1
...
def calculateDistance(sim, output):
...
def mergeParticles(sim, time, seed, logfile):
    ## Find two closest particles and return the planets
    cp1, cp2 = calculateDistance(sim, "planets")
...
def run_sim(seed):
    ...
    ## Check if heartbeat file exists, if not, create a blank text file    
    if (os.path.exists(heartbeatPath) == False):
        np.savetxt(heartbeatPath, emp)
